...
class MiniChart {
    ...
    draw() {
        ...
        const scaleX = (i) => margin + (i / (labels.length - 1)) * (W - margin * 2);
        const scaleY = (v) => H - margin - ((v - minY) / (maxY - minY)) * (H - margin * 2);
        ...
        // === MARCAJE AXA X ===
        // Lățimea minimă necesară pentru o etichetă (în px)
        const minLabelSpacing = 50;

        // Câte etichete încap pe axă
        const maxXTicks = Math.floor((W - margin * 2) / minLabelSpacing);
        const stepX = Math.ceil(labels.length / maxXTicks);
        ...
        for (let i = 0; i < labels.length; i += stepX) {
            const px = scaleX(i);
            ctx.fillText(labels[i], px, H - margin + 12);
        }

        // === MARCAJE AXA Y ===
        // Înălțimea minimă necesară pentru o etichetă Y (în px)
        const minYLabelSpacing = 30;
